-- Demo data: one matter, three NDAs, one review already part-filled.
--
-- The page text below is real enough to exercise the citation check: the
-- "answered" quotes are genuinely present in it, and the "rejected" one is
-- genuinely absent — which is why it is stored as unresolved. Re-running the
-- review against this data reproduces the same verdicts.
--
-- The R2 keys point at nothing, so "Open document" will 404 on seeded rows.
-- Upload a real file to exercise that path.
...
insert into reviews (id, matter_id, name, status) values
  ('99999999-9999-4999-8999-999999999999', '11111111-1111-4111-8111-111111111111', 'NDA review — round 1', 'running');
